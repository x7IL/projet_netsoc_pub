...
//                insert_fields('post', ["ID_user" => $id_user, "post" => $modifier_ver]);  //mettre l'id user dans la requets
                ...
                $mysqli->query("INSERT INTO post (post,ID_user,username_dest) VALUES ('$modifier_ver','{$result_can['id']}','{$result_can['username']}')") or die($mysqli->error);
            }
        }
        ?>
    </form>
    ...
    foreach ($coms as $com){
        ...
        if ($ID_user!=NULL) {
            ...
            <div style="background-color: #FAFAFA; border: solid 1px dodgerblue; margin-top: 1%;">
                <b style=" max-width: 99%; word-wrap: break-word; "><?=$username; ?> </b>
            ...
            if ($res) {
                ...
                if($result_can){?>
                    <form method="POST">
                        ...
                        <input type="hidden" name="username_destination" value="<?php echo $com['username_dest']?>"/>
